...
const STATUS_CONFIG: Record<OrderStatus, { label: string; color: string; bg: string }> = {
  ...
  cancelled_by_user: { label: "Cancelled", color: Colors.danger, bg: Colors.dangerSoft },
  cancelled_by_technician: { label: "Cancelled by tech", color: Colors.danger, bg: Colors.dangerSoft },
    ...
    >
      <View className="flex-row items-center gap-3">
        ...
        {order.technician_image ? (
          ...
          >
            <Text style={{ fontFamily: "GoogleSans_700Bold", fontSize: 16, color: Colors.surfaceBase }}>
              {initials}
            </Text>
          </View>
        )}
      ...
      {/* Date row */}
      <View className="mt-3 flex-row items-center justify-between border-t pt-3" style={{ borderColor: Colors.borderDefault }}>
        <Text style={{ fontSize: 12, color: Colors.textMuted }}>
          {formatDate(order.scheduled_date)}
        </Text>
        <Text style={{ fontSize: 12, color: Colors.primary, fontFamily: "GoogleSans_600SemiBold" }}>
          View Details
        </Text>
      </View>
    </TouchableOpacity>
