...
function normalizePayload(data: unknown): NotificationNavigationPayload | undefined {
...
export function useNotificationRouting(enabled: boolean): void {
  ...
  useEffect(() => {
    ...
    const handleResponse = (response: Notifications.NotificationResponse) => {
      ...
      const payload = normalizePayload(response.notification.request.content.data);
      if (!payload?.orderId) return;
      logger.info("PushNotifications", "Routing from notification tap", payload);
    ...
    void Notifications.getLastNotificationResponseAsync()
      ...
      .catch((error) => {
        logger.warn("PushNotifications", "Failed to read last notification response", {
