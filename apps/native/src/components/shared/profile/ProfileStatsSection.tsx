...
export default function ProfileStatsSection({ bookings, completed }: ProfileStatsSectionProps) {
  return (
    <View className="flex-row gap-3 px-5" style={{ marginTop: -16 }}>
      <StatsCard icon={CalendarDays} iconBg={Colors.cyan} count={bookings} label="Bookings" />
      <StatsCard icon={BadgeCheck} iconBg={Colors.purple} count={completed} label="Completed" />
    </View>
  );
}
