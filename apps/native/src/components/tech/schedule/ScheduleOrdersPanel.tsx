...
export default function ScheduleOrdersPanel({ orders }: ScheduleOrdersPanelProps) {
  ...
  return (
    <View className="mt-2.5 overflow-hidden rounded-[14px] border border-success-alt/25 bg-order-green-bg">
      <TouchableOpacity
      ...
      >
        <View className="flex-row items-center gap-2">
          ...
          <Text className="font-semibold text-[13px] text-order-green-dark">
            {orders.length} order{orders.length > 1 ? 's' : ''} this day
          </Text>
        </View>
        {expanded
          ? <ChevronUp size={18} color={Colors.orderGreenDark} strokeWidth={2} />
          : <ChevronDown size={18} color={Colors.orderGreenDark} strokeWidth={2} />}
      </TouchableOpacity>
