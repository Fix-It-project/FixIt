...
export default function ScheduleDayPanel({
  ...
  return (
    <View className="mx-3 mt-3 rounded-2xl border border-edge-light bg-surface-gray p-3.5">
      ...
      {isSelectedDatePast ? (
        ...
        <>
          <Text className="mt-1 text-[13px] text-unavailable-orange">
            🚫 Marked as unavailable (override)
          </Text>
          <TouchableOpacity
            onPress={onRemoveOverride}
            disabled={isDeletingException}
            className={`mt-2.5 items-center rounded-xl border border-unavailable-orange py-2.5 ${
              isDeletingException ? 'bg-edge-light' : 'bg-unavailable-orange-bg'
            }`}
          >
            <Text
              className={`font-semibold text-[13px] ${
                isDeletingException ? 'text-content-muted' : 'text-unavailable-orange'
              }`}
            >
        ...
        <>
          ...
          {canMarkUnavailable && (
            <TouchableOpacity
              ...
              className={`mt-2.5 items-center rounded-xl border border-edge-light py-2.5 ${
                isAddingException ? 'bg-edge-light' : 'bg-surface-white'
              }`}
            >
              <Text
                className={`font-semibold text-[13px] ${
