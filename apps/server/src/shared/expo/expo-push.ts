...
// Parse a completed Expo /push/send HTTP response into a ticket. A bad HTTP
// status or a ticket-level error (e.g. DeviceNotRegistered) is final → throws.
async function parseExpoSendResponse(
	response: Response,
...
export async function sendExpoPush(
	...
	for (let attempt = 0; attempt < PUSH_MAX_ATTEMPTS; attempt += 1) {
		...
		return await parseExpoSendResponse(response);
	}
