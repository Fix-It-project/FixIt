...
describe('RescheduleRequestBodySchema', () => {
  it('accepts valid body', () => {
    const r = RescheduleRequestBodySchema.safeParse({ proposed_scheduled_date: '2026-06-01', reason: 'sick' });
  ...
  it('rejects empty reason', () => {
    const r = RescheduleRequestBodySchema.safeParse({ proposed_scheduled_date: '2026-06-01', reason: '' });
    expect(r.success).toBe(false);
  });
  it('rejects reason >500 chars', () => {
    const r = RescheduleRequestBodySchema.safeParse({ proposed_scheduled_date: '2026-06-01', reason: 'a'.repeat(501) });
    expect(r.success).toBe(false);
  });
});
