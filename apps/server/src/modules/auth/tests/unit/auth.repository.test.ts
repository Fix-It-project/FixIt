...
describe("AuthRepository", () => {
	...
	describe("signUp", () => {
		...
		it("should throw custom message when user already registered", async () => {
			...
			await expect(
				repo.signUp({ email: "a@b.com", password: "p" }),
	...
	describe("signIn", () => {
		...
		it("should map invalid credentials to unauthorized AppError", async () => {
			mockAuth.signInWithPassword.mockResolvedValue({
				data: null,
	...
	describe("resetPassword", () => {
		...
		it("should throw on error", async () => {
			...
			await expect(repo.resetPassword("123")).rejects.toThrow(
				"Password too weak",
			);
		});
	});
	...
	describe("signOut", () => {
		...
		it("should throw on error", async () => {
			...
			await expect(repo.signOut("bad-token")).rejects.toThrow(
				"Session not found",
			);
		});
	});
	...
	describe("refreshToken", () => {
		...
		it("should throw on expired refresh token", async () => {
			...
			await expect(repo.refreshToken("expired-tok")).rejects.toThrow(
