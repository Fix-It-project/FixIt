...
export class TechnicianCalendarController {
	...
	async getCalendar(req: Request, res: Response) {
		try {
			...
			const entries = await technicianCalendarService.getCalendar(
				technicianId,
				{ from, to },
			);
	...
	async getPublicSchedule(req: Request, res: Response) {
		try {
			...
			const templates = await technicianCalendarService.getTemplates(
				technicianId,
				false,
			);
			const exceptions = await technicianCalendarService.getCalendar(
				technicianId,
				{ from, to },
			);
	...
	async getBookedSlots(req: Request, res: Response) {
		try {
			...
			const slots = await technicianCalendarService.getBookedSlots(
				technicianId,
				{ from, to },
			);
