...
describe("applyReportFlagsToOrders", () => {
	it("marks orders the viewer already has an open report on", () => {
		const orders = [baseOrder("order-1"), baseOrder("order-2")];

		const result = applyReportFlagsToOrders(orders, new Set(["order-2"]));

		expect(result).toMatchObject([
			{ id: "order-1", has_open_report: false },
			{ id: "order-2", has_open_report: true },
		]);
	});

	it("preserves other order fields", () => {
		const orders = [baseOrder("order-1", "cancelled")];

		const result = applyReportFlagsToOrders(orders, new Set(["order-1"]));
