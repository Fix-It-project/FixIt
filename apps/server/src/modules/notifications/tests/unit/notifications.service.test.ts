...
const { mockRepo, mockExpo } = vi.hoisted(() => ({
	mockRepo: {
		...
		deleteDevicesForRecipient: vi.fn(),
...
describe("NotificationsService", () => {
	...
	it("unregisterDevice deactivates the device by token + recipient", async () => {
		...
		expect(mockRepo.deactivateByExpoPushToken).toHaveBeenCalledWith(
			"user",
			"user-1",
	...
	it("removeAllForRecipient hard-deletes devices and preferences", async () => {
		await service.removeAllForRecipient("technician", "tech-1");

		expect(mockRepo.deleteDevicesForRecipient).toHaveBeenCalledWith(
			"technician",
			"tech-1",
		);
		expect(mockRepo.deletePreferences).toHaveBeenCalledWith(
			"technician",
			"tech-1",
		);
	});

	it("getUnreadCount validates the recipient then returns the count", async () => {
		mockRepo.recipientExists.mockResolvedValue(true);
		mockRepo.countUnreadForRecipient.mockResolvedValue(7);

		const count = await service.getUnreadCount("user", "user-1");

		expect(mockRepo.recipientExists).toHaveBeenCalledWith("user", "user-1");
		expect(count).toBe(7);
	});

	it("getUnreadCount throws 404 when the recipient does not exist", async () => {
		mockRepo.recipientExists.mockResolvedValue(false);

		await expect(service.getUnreadCount("user", "ghost")).rejects.toMatchObject(
			{ status: 404 },
		);
		expect(mockRepo.countUnreadForRecipient).not.toHaveBeenCalled();
	});
});
