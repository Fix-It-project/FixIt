...
import {
	type DocumentFiles,
	storageRepository,
} from "../../shared/storage/storage.repository.js";
import {
	addressesRepository,
...
export class TechnicianAuthService {
	...
	async signUp(
		...
		const documentUrls = await storageRepository.uploadDocuments(
			technicianId,
			files,
		);
		...
		return {
			...
			message:
				"Technician registered successfully. Please sign in to continue.",
		};
	}
	...
	async cancelApplication(email: string, password: string) {
		...
		const cleanup: Array<[string, () => Promise<unknown>]> = [
			...
			[
				"notifications",
				() => notificationsService.removeAllForRecipient("technician", userId),
			],
		...
		logger.info(
			{ technicianId: userId },
			"[technician-auth] application cancelled",
		);
