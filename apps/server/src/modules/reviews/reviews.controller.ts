...
export class ReviewsController {
	async createReview(req: Request, res: Response) {
		try {
			const review = await reviewsService.createReviewForUser(
				req.user!.id,
				req.body,
			);
	...
	async getMyTechnicianReviews(req: Request, res: Response) {
		try {
			const { limit, offset } = req.query as unknown as {
				limit: number;
				offset: number;
			};
			const reviews = await reviewsService.getReviewsForAuthenticatedTechnician(
				req.technician!.id,
				limit,
				offset,
			);
