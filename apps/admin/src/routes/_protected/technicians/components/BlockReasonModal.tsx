...
export function BlockReasonModal({ techName, open, onClose, onConfirm }: BlockReasonModalProps) {
	...
	return (
		<Dialog open={open} onOpenChange={onClose}>
			<DialogContent className="max-w-sm">
				<DialogHeader>
					<DialogTitle>Block {techName}?</DialogTitle>
				</DialogHeader>

				<div className="flex flex-col gap-3">
					<p className="text-sm text-muted-foreground">Select a reason for blocking this technician. They will not receive new orders while blocked.</p>
					<RadioGroup value={selected} onValueChange={setSelected} className="flex flex-col gap-2">
						...
						<div className="flex items-center gap-2">
							...
							<Label htmlFor="custom" className="text-sm cursor-pointer">Custom reason…</Label>
						</div>
					</RadioGroup>
