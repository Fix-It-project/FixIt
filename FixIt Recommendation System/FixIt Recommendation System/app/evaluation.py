        ...
        for _, booking in completed_test.iterrows():
            ...
            try:
                recommendations, _, _ = hybrid_engine.recommend(
                    ...
                    latitude=lat,
                    longitude=lon,
                    radius_km=25.0,                       # ← wide enough for 5 clusters
                    top_k=max(EVAL_K_VALUES),
                )
            except Exception as e:
                if n_evaluated == 0:
                    logger.warning("Evaluation error (first failure): %s", e)
                continue

            rec_tids = [str(r.technician_id) for r in recommendations]
            rec_cats = [r.category for r in recommendations]
            relevant_str = {str(x) for x in relevant}

            for k in EVAL_K_VALUES:
                all_precisions[k].append(precision_at_k(rec_tids, relevant_str, k))
                all_cat_hits[k].append(
                    category_hit_at_k(rec_cats, booked_cat, k)
                )
            all_rrs.append(reciprocal_rank(rec_tids, relevant_str))
            n_evaluated += 1
